...
function Get-ArkaNormalizedToken {
  param([string]$Value)
...
function Get-ArkaRepoRoot {
  ...
  try {
    $resolved = Resolve-Path -LiteralPath $BaseScriptRoot
    $currentPath = if ($resolved -is [System.Management.Automation.PathInfo]) { $resolved.Path } else { [string]$resolved }
    while ($currentPath) {
      $providerMarker = Join-Path $currentPath '.openAi-provider'
      $gitMarker = Join-Path $currentPath '.git'
      if (Test-Path -LiteralPath $providerMarker) { return (Resolve-Path -LiteralPath $currentPath).Path }
      if (Test-Path -LiteralPath $gitMarker) { return (Resolve-Path -LiteralPath $currentPath).Path }

      $parentPath = Split-Path -Path $currentPath -Parent
      if (-not $parentPath -or $parentPath.Equals($currentPath, [System.StringComparison]::OrdinalIgnoreCase)) {
        break
      }
...
function Get-ArkaProviderCatalog {
  ...
  Get-ChildItem -Path $providerRoot -Directory -Force | ForEach-Object {
    if ($_.Name -match '^\.(?<provider>[^-]+)-(?<role>.+)$') {
      ...
      $roleToken = Get-ArkaNormalizedToken $roleFolder
...
function Find-ArkaCatalogEntry {
  ...
  $agentToken = Get-ArkaNormalizedToken $AgentName
  $agentSlugToken = Get-ArkaNormalizedToken $AgentSlug
  ...
  foreach ($entry in $candidates) {
    ...
    if ($roleToken -and $agentToken -and $roleToken -eq $agentToken) { return $entry }
    if ($roleToken -and $agentSlugToken -and $roleToken -eq $agentSlugToken) { return $entry }
    if ($agentToken -and $roleToken -and $agentToken.Contains($roleToken)) {
      if (-not $best -or $roleToken.Length -lt $best.RoleToken.Length) {
        $best = $entry
      }
    } elseif ($roleToken -and $agentToken -and $roleToken.Contains($agentToken)) {
      if (-not $best -or $roleToken.Length -lt $best.RoleToken.Length) {
        $best = $entry
      }
    } elseif ($AgentSlug -and $roleKey -and $AgentSlug.Contains($roleKey)) {
...
function Resolve-ArkaRoleFolder {
  ...
  $roleToken = Get-ArkaNormalizedToken $Role
  ...
  $match = $ProviderCatalog | Where-Object {
    $_.Provider.ToLowerInvariant() -eq $providerKey -and (
      ...
      $_.RoleKey -eq $roleKey -or
      ($roleToken -and $_.RoleToken -eq $roleToken)
    )
  }

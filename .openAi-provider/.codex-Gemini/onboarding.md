...
**Onboarding YAML (canonique, client)** : `ARKA_OS/ARKA_AGENT/clients/<CLIENT>/agents/gemini/onboarding.yaml`
...
4. **Wake-up & règles (Gemini)** : `.openAi-provider/.codex-Gemini/WAKEUP-LINK.md` — Retrouve intents, guardrails, références messaging.
...
- **Messagerie** : exécute `arkamsg pull --agent gemini-owner-sourcing` dès l'ouverture de session, vérifie si des messages sont présents puis applique la procédure ARKORE20.
...
- `dir` : `ARKA_META/.system/.mem/gemini/`
- `index` : `ARKA_META/.system/.mem/gemini/index.json`

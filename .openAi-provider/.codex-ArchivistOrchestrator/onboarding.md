...
**Onboarding YAML (canonique, client)** : `ARKA_OS/ARKA_AGENT/clients/<CLIENT>/agents/archivistorchestrator/onboarding.yaml`
...
4. **Wake-up & règles (ArchivistOrchestrator)** : `.openAi-provider/.codex-ArchivistOrchestrator/WAKEUP-LINK.md` — Retrouve intents, guardrails, références messaging.
...
- **Messagerie** : exécute `arkamsg pull --agent arka-agent01-arka-archivist-orchestrator` dès l'ouverture de session, vérifie si des messages sont présents puis applique la procédure ARKORE20.
...
- `dir` : `ARKA_META/.system/.mem/archivistorchestrator/`
- `index` : `ARKA_META/.system/.mem/archivistorchestrator/index.json`

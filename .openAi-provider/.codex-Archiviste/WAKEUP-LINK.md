...
- Vérifier que le wake-up (`ARKA_OS\ARKA_AGENT\client\acme\wakeup\ARKAA08-WAKEUP-CORE_ARCHIVIST.yaml`) référence bien `messaging.general_ref` et `messaging.inbox_ref` avant d'exécuter une action.
...
**link:**
  ...
  **ref:** `ARKA_OS\ARKA_AGENT\client\acme\wakeup\ARKAA08-WAKEUP-CORE_ARCHIVIST.yaml`   # chemin/référence ARKA_OS
**policy**:
  - repo_scan: false
...
- **Onboarding YAML (client)** → `ARKA_OS/ARKA_AGENT/clients/<CLIENT>/agents/archiviste/onboarding.yaml`

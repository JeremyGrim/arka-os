...
**Onboarding YAML (canonique, client)** : `ARKA_OS/ARKA_AGENT/clients/<CLIENT>/agents/missionqualifier/onboarding.yaml`
...
4. **Wake-up & règles (MissionQualifier)** : `.openAi-provider/.codex-MissionQualifier/WAKEUP-LINK.md` — Retrouve intents, guardrails, références messaging.
...
- **Messagerie** : exécute `arkamsg pull --agent mission-qualifier` dès l'ouverture de session, vérifie si des messages sont présents puis applique la procédure ARKORE20.
...
- `dir` : `ARKA_META/.system/.mem/missionqualifier/`
- `index` : `ARKA_META/.system/.mem/missionqualifier/index.json`

...
- Exécuter `arkamsg pull --agent arka-agent14-security-compliance-architect` dès l'ouverture de session, vérifier s'il existe des messages puis suivre la procédure ARKORE20 pour chacun.
- Ne jamais utiliser le user comme proxy : envoie toujours via messagerie et cite le `message_id` dans la réponse.
- Vérifier que le wake-up (`ARKA_OS\ARKA_AGENT\client\acme\wakeup\ARKAA08-WAKEUP-SECURITY_COMPLIANCE_ARCHITECT.yaml`) référence bien `messaging.general_ref` et `messaging.inbox_ref` avant d'exécuter une action.
...
**link:**
  ...
  **ref:** `ARKA_OS\ARKA_AGENT\client\acme\wakeup\ARKAA08-WAKEUP-SECURITY_COMPLIANCE_ARCHITECT.yaml`   # chemin/référence ARKA_OS
**policy**:
  - repo_scan: false
...
- **Onboarding YAML (client)** → `ARKA_OS/ARKA_AGENT/clients/<CLIENT>/agents/securitycompliance/onboarding.yaml`
